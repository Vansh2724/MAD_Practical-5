package com.example.mad_prac5_89_5b6
...
class MainActivity : AppCompatActivity() {
    ...
    private fun implicitIntent(){
        findViewById<Button>(R.id.btn_browse).setOnClickListener {
            ...
            Intent(
                Intent.ACTION_VIEW,
                Uri.parse(findViewById<EditText>(R.id.edit_web_url).text.toString())).also { startActivity(it) }
        }
        findViewById<Button>(R.id.btn_call).setOnClickListener {
            val number=findViewById<EditText>(R.id.edit_phone).text.toString().trim()
